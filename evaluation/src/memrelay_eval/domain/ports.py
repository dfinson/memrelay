...
class LedgerPort(Protocol):
    ...
    def attempt_terminals_for(self, run_id: RunId) -> Sequence[AttemptTerminal]: ...
    def retry_lineage_for(self, run_id: RunId) -> Sequence[tuple[AttemptId, AttemptId]]: ...
    ...
    def record_reconciliation_authority(self, authority: object) -> bool: ...
    ...
    def cost_ledger_entries_for(self, attempt_id: AttemptId) -> Sequence[CostLedgerLink]: ...
    def monetary_views_for(self, attempt_id: AttemptId) -> Sequence[MonetaryViewLink]: ...
...
class JudgeProcessPort(Protocol):
    """A fresh, credential-isolated process boundary for one blinded judge session."""
...
class DirectEngineGraphClaimPort(Protocol):
    def claim(self, graph_path_digest: str, attempt_id: AttemptId) -> None: ...


class DirectEngineTreatmentPort(Protocol):
...
class ProcessLauncherPort(Protocol):
    """Launches a role once inside its explicitly owned attempt boundary."""
