...
def test_network_namespace_blocks_listener_and_host_escapes(tmp_path: Path) -> None:
    ...
    script = (
        "import json, os, pathlib, shutil, socket, ssl, subprocess, sys\n"
        f"port={port}\n"
        ...
        "popen=captured_popen([sys.executable,'-c',"
        "'import socket,sys;sys.exit(0 if socket.socket().connect_ex((\"127.0.0.1\","
        "+str(port)+')) else 1)']).wait() == 0\n"
        "system=captured_system(sys.executable + ' -c \"import socket,sys;"
        "sys.exit(0 if socket.socket().connect_ex((\\'127.0.0.1\\','"
        "+str(port)+')) else 1)\"') != 0\n"
