...
def _preflight(**overrides: object) -> object:
...
def test_preflight_rejects_every_nonfrozen_strategy(strategy: str) -> None:
    with pytest.raises(ConformancePauseError):
        _preflight(llm_strategy=strategy)


def test_preflight_requires_normalized_exact_direct_openai_url() -> None:
    evidence = _preflight(openai_base_url="HTTPS://API.OPENAI.COM/v1/")
    ...
    with pytest.raises(ConformancePauseError):
        _preflight(openai_base_url="https://api.openai.com.evil.invalid/v1")
    daemon, agent, mcp = build_framework_process_environments(
...
def test_preflight_detects_normalized_credential_leaks(variable: str) -> None:
    daemon, agent, mcp = build_framework_process_environments()
