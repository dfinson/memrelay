...
- 2026-08-11T15:51:00-04:00 - Independent review identified two mechanical blockers and both were repaired. Generic wrapped commands now retain the original command diagnostic in an optional `prior_error_code`, emit exactly one canonical failed terminal manifest to stdout when immutable manifest publication fails, and raise a typed fail-closed publication error rather than leaking filesystem exceptions or a success-shaped record. Successful commands retain their established stdout contracts while persisting the standard immutable manifest. `StageBundleStore` staged paths now include a cryptographically unique UUID nonce in addition to the process ID, preventing same-process thread collisions; threaded identical seals produce one `sealed` plus reuses, while differing seals fail closed with `stage_bundle_mutation`. Focused blocker and affected integration suites: 133 passed. Full evaluator suite: 1421 passed, 46 skipped, 2 failures; the pre-existing SDK ToolResult-shape failure remains, and an unrelated catalog interruption test passed on immediate isolated rerun (6 passed).
